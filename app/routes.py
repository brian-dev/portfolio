...
def index():
    ...
    posts = db.paginate(current_user.following_posts(), page=page, per_page=app.config['POSTS_PER_PAGE'],
                        error_out=False)
...
def user(username):
    ...
    prev_url = url_for('user', username=username, page=posts.has_prev) if posts.has_prev else None
